                          ...
                          <div class="modal-dialog">
                              <div class="modal-content">
                                  <div class="modal-header">
                                        ...
                                        <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> اضافة مستخدم</h4>
                                  </div>
                                  <form role="form"  method="POST" class="addForm" action="{{ url('/customer/store') }}" data-toggle="validator"  enctype="multipart/form-data" >
                      ...
                      <div class="modal fade" id="editcountryModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
                          <div class="modal-dialog">
                              <div class="modal-content">
                                  ...
                                  <form role="form"  method="POST" class="editForm" data-id="" action="{{ url('/customer/update') }}" data-toggle="validator" enctype="multipart/form-data">
                                      <div class="modal-body">
                                        @include('customer.form')
                                      </div>
                                      <div class="modal-footer">
		...
		function populateForm(response, frm)
    {
						...
						$.ajax({
								...
								success: function(res){
		 		                     ...
		 		                     $('.alerts-list').append(
		 		                         '<li>\
		 		                             <div class="alert alert-success alert-dismissable">\
		 		                                 <i class="icon-check-sign"></i> Advertise has been successfully updated!\
		 		                                 <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>\
		 		                             </div>\
		 		                         </li>');
												oTable.ajax.reload();
       ...
       //Edit Form
           ...
           $('.editForm')[0].reset();
		         ...
		         self.button('loading');
		         $.ajax({
		             url: "{{ url('customer') }}" + "/" + self.data('id') + "/edit" ,
		             type: "GET",
		             success: function(res){
		                 self.button('reset');
